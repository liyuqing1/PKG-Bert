...
def mask_seq(src, vocab_size):
        ...
        return:
            src: a list of token ids with [mask]
            tgt_mlm: the masked token's id, the place not masked is PAD_ID
        """
        tgt_mlm = []
        for (i, token) in enumerate(src):
            if token == CLS_ID or token == SEP_ID:#特殊字符不掩盖
            ...
            if prob < 0.15:
                ...
                if prob < 0.8:#mask
                    src[i] = MASK_ID
                elif prob < 0.9:#place random character
                    while True:
